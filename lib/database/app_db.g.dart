    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Note` (`nId` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `txtTitle` TEXT NOT NULL, `txtDescription` TEXT NOT NULL)');
...
class _$NoteDao extends NoteDao {
  ...
  Future<List<Note>> findAllNotes() async {
    return _queryAdapter.queryList('SELECT * FROM Note',
        mapper: (Map<String, Object?> row) => Note(row['nId'] as int,
            row['txtTitle'] as String, row['txtDescription'] as String));
  }

  @override
  Stream<Note?> findNoteById(int nId) {
    return _queryAdapter.queryStream('SELECT * FROM Note WHERE nId = ?1',
        mapper: (Map<String, Object?> row) => Note(row['nId'] as int,
            row['txtTitle'] as String, row['txtDescription'] as String),
        arguments: [nId],
        queryableName: 'Note',
